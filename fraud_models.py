...
OUT_FILE   = os.path.join(BASE, "fraud_flags.csv")
SUM_FILE   = os.path.join(BASE, "fraud_summary.txt")
...
def _build_vendor_alias_map(vendor_names_series: pd.Series) -> dict:
    """
    Cluster similar vendor names using Sentence Transformer embeddings + Union-Find.

    This catches fraud patterns like:
      'SHARMA CONSTRUCTION', 'S. CONSTRUCTIONS PVT LTD', 'SHARMA CONST. & CO.'
    all resolving to one canonical entity -- enabling true concentration analysis.

    Returns: dict {raw_vendor_name -> canonical_cluster_name}
    Falls back to exact-match (identity map) if sentence-transformers not installed.
    """
    unique_vendors = vendor_names_series.dropna().unique().tolist()
    n = len(unique_vendors)
    print(f"  Unique vendor names to resolve: {n:,}")

    # Bug A guard: NearestNeighbors requires n_neighbors >= 1.
    # If there are 0 or 1 unique vendors (testing / tiny filtered slice),
    # skip clustering entirely -- each name is its own canonical.
    if n <= 1:
        return {v: v for v in unique_vendors}
    ...
    # Step 1: Encode all vendor names into unit-length vectors
    print("  Loading all-MiniLM-L6-v2 and encoding vendor names...")
    ...
    # Step 2: Find high-similarity pairs via Approximate Nearest Neighbours
    # For normalised vectors: euclidean_dist = sqrt(2 * (1 - cosine_sim))
    euc_threshold = np.sqrt(2.0 * (1.0 - VENDOR_SIMILARITY_THRESHOLD))

    print(f"  Finding alias pairs (cosine >= {VENDOR_SIMILARITY_THRESHOLD})...")
    ...
    for members in clusters.values():
        canonical = max(members, key=len)  # longest = full legal name, not abbreviation
        for vendor in members:
            vendor_to_canonical[vendor] = canonical

    n_clusters = len(clusters)
    n_aliases  = n - n_clusters
    print(f"  Resolved {n:,} names -> {n_clusters:,} unique entities ({n_aliases:,} aliases detected)")
...
def model2_vendor_identity_resolution(exp: pd.DataFrame, alloc: pd.DataFrame) -> pd.DataFrame:
    ...
    alias_map = _build_vendor_alias_map(exp["vendor_name"])

    exp2 = exp.copy()
    exp2["canonical_vendor"] = exp2["vendor_name"].map(alias_map).fillna(exp2["vendor_name"])
...
def model3_compliance_rules(san: pd.DataFrame, exp: pd.DataFrame,
    ...
    df = san[["work_id", "mp_name", "mp_number", "sanction_amount",
              "sanction_date", "work_status", "implausible_amount_flag"]].copy()
    ...
    # Rule 3: Completed work has no photo evidence uploaded
    ...
    return df[["work_id", "compliance_score", "m3_reason",
               ...
               "rule_missing_photo", "rule_early_payment", "rule_implausible"]].copy()
...
def model5_ensemble(m1: pd.DataFrame, m2: tuple, m3: pd.DataFrame,
    ...
    base = base.merge(
        m3[["work_id", "compliance_score", "m3_reason",
            "rule_overspend", "rule_mp_over_budget",
            "rule_missing_photo", "rule_early_payment", "rule_implausible"]],
        on="work_id", how="left"
    )
    ...
    # ----------------------------------------------------------------
    # Dynamic percentile-based risk bands.
    # Fixed thresholds (e.g. 80 = CRITICAL) fail when the weighted
    # average of four percentile scores mathematically never reaches 80.
    # Instead, top 1% of works = CRITICAL, next 9% = HIGH, next 20% = MEDIUM.
    # This guarantees meaningful tier distribution on any dataset.
    # ----------------------------------------------------------------
    p_critical = base["risk_score"].quantile(0.99)
    ...
    base["risk_label"] = base["risk_score"].apply(risk_label_dynamic)

    print(f"  Risk thresholds: CRITICAL>={p_critical:.1f}, HIGH>={p_high:.1f}, MEDIUM>={p_medium:.1f}")
