...
- **Full-Lifecycle Cumulative Tranche Gatekeeper with Utilization Certificate Verification:**
  Enforces recursive compliance across all $N$ installments:
  $$\frac{\sum_{i=1}^{k-1} \text{Verified Spent}_i}{\sum_{i=1}^{k-1} \text{Disbursed}_i} \ge 0.75 \quad \forall k \ge 2$$
  Cross-checks whether the statutory Form II (Utilization Certificate) was logged in the document repository prior to release of tranche $k$.

**4. Deep Technical & Architectural Specification:**
- **Recursive Audit Algorithm:**
  ```python
  def audit_cumulative_tranche_gate(work_id: str, tranches: list[dict]):
      # tranches sorted by tranche_number
      cum_disbursed = 0.0
      cum_spent = 0.0
      violations = []
...
**4. Deep Technical & Architectural Specification:**
- **Formulas & Code:**
  $$\text{Stall Index} = \frac{\text{Days Since Last Milestone Update}}{\text{Permissible Execution Days (Default 365)}}$$
  $$\text{Inflation Loss} = \text{Disbursed Amount} \times \left( \frac{\text{CII}_{\text{current}}}{\text{CII}_{\text{sanction}}} - 1 \right)$$
  ```python
  def evaluate_work_abandonment(work: dict, cii_table: dict):
      ...
      if work['physical_progress_pct'] < 100.0 and days_stalled > 180:
          escalation_loss = work['expenditure_amount'] * 0.06 * (days_stalled / 365.0) # approx 6% annual inflation
          return {
...
- **Verification Matrix:**
  ...
  def verify_bank_compliance(ifsc_code: str, agency_name: str):
...
- **Feature Pipeline:**
  ...
  def build_stratified_features(df_works: pd.DataFrame):
      ...
      df_works['iso_raw_score'] = -iso.fit(X).decision_function(X) # Higher = more anomalous
      # Min-max scale to 0-1
      df_works['norm_iso_score'] = (df_works['iso_raw_score'] - df_works['iso_raw_score'].min()) / (df_works['iso_raw_score'].max() - df_works['iso_raw_score'].min() + 1e-6)
      return df_works
  ```
...
- **Mathematical Blueprint:**
  ...
  def compute_robust_benford(amounts: list[float], min_n=100):
      ...
      # Critical value at p=0.05, df=8 is 15.51
